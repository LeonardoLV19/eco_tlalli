...
const TabBar = ({ role }: TabBarProps) => {
  const navItems = {
    ...
    admin: [{ label: "Vendedores", active: true }, { label: "Instalaciones" }],
  };
  ...
  const icons = {
    cliente: [
      ...
      <Link key="user" href="/login-customer">
        <PersonIcon
          sx={{ fontSize: 30, color: "#5DAF5D", cursor: "pointer" }}
        />
      </Link>,
    ],
    vendedor: [
    ...
    admin: [<PersonIcon key="user" sx={{ fontSize: 30, color: "#5DAF5D" }} />],
  };
    ...
    <header className="w-full bg-[#F5F7F9] border-t-[3px] border-[#F5F7F9]">
      <div className="max-w-7xl mx-auto flex items-center justify-between px-15 py-4">
        ...
        <div className="flex items-center space-x-2">
          ...
          <span className="font-extrabold text-2xl text-[#1E1E1E]">
            EcoTlalli
          </span>
        </div>
        ...
        {/* Íconos */}
        <div className="flex space-x-4 items-center">{icons[role]}</div>
      </div>
    </header>
  );
};
